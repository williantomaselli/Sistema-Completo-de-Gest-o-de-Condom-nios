...
// recuperar todos os tipos de manutenção
app.get('/api/tipos_manutencao', (req, res) => {
  db.query(
    'SELECT id, descricao FROM tipo_manutencao ORDER BY descricao',
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
    }
...
// recuperar manutenções existentes (para checar duplicidade)
app.get('/api/manutencoes', (req, res) => {
  const sql = 'SELECT id, tipo_id, data, local FROM manutencao';
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(results);
  });
...
app.post('/api/manutencoes', (req, res) => {
  const { tipo_id, data, local, descricao } = req.body;
  if (!tipo_id || !data || !local) {
    return res.status(400).json({ error: 'Tipo, data e local são obrigatórios' });
  }
  // checar duplicidade: mesmo tipo_id e data e local
  db.query(
    'SELECT id FROM manutencao WHERE tipo_id = ? AND data = ? AND local = ?',
    [tipo_id, data, local],
    (err1, rows) => {
      if (err1) return res.status(500).json({ error: err1.message });
      if (rows.length) {
        return res.status(409).json({ error: 'Manutenção deste tipo, data e local já registrada' });
      }
      // inserir
      db.query(
        'INSERT INTO manutencao (tipo_id, data, local, descricao) VALUES (?, ?, ?, ?)',
        [tipo_id, data, local, descricao || null],
        (err2, result) => {
          if (err2) return res.status(500).json({ error: err2.message });
          res.status(201).json({ id: result.insertId });
        }
...
app.put('/api/manutencoes/:id', (req, res) => {
  ...
  db.query(
    'SELECT id FROM manutencao WHERE tipo_id = ? AND data = ? AND local = ? AND id <> ?',
    [tipo_id, data, local, id],
    (e1, rows) => {
      if (e1) return res.status(500).json({ error: e1.message });
      if (rows.length) return res.status(409).json({ error: 'Já existe manutenção com esses dados' });
      // executar update
      db.query(
        'UPDATE manutencao SET tipo_id=?, data=?, local=?, descricao=? WHERE id=?',
        [tipo_id, data, local, descricao || null, id],
        (e2, result) => {
          if (e2) return res.status(500).json({ error: e2.message });
          if (!result.affectedRows) return res.status(404).json({ error: 'Manutenção não encontrada' });
          res.json({ message: 'Atualizado com sucesso' });
        }
      );
    }
...
app.delete('/api/manutencoes/:id', (req, res) => {
  db.query('DELETE FROM manutencao WHERE id = ?', [req.params.id], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!result.affectedRows) return res.status(404).json({ error: 'Manutenção não encontrada' });
    res.json({ message: 'Excluído com sucesso' });
  });
